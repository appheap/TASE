...
class Audio(BaseDocument):
    ...
    @staticmethod
    def get_key_from_audio(audio: 'elasticsearch_models.Audio', telegram_client_id: int):
        return f'{str(telegram_client_id)}:{audio.file_unique_id}:{audio.chat_id}:{audio.message_id}'

    @staticmethod
    def parse_from_message(message: 'pyrogram.types.Message', telegram_client_id: int) -> Optional['Audio']:
