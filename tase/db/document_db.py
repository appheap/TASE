...
class DocumentDatabase:
    ...
    def __init__(
        ...
        for doc in docs:
            if not self.db.has_collection(doc._doc_collection_name):
                setattr(
                    self,
                    doc._doc_collection_name,
                    self.db.create_collection(doc._doc_collection_name),
                )
            else:
                setattr(
                    self,
                    doc._doc_collection_name,
                    self.db.collection(doc._doc_collection_name),
                )
    ...
    def create_audio(
        ...
        audio, successful = Audio.create(
            self.doc_audios, Audio.parse_from_message(message, telegram_client_id)
        )
        return audio
    ...
    def get_or_create_audio(
        ...
        audio = Audio.find_by_key(
            self.doc_audios, Audio.get_key(message, telegram_client_id)
        )
        if not audio:
    ...
    def get_audio_file_from_cache(self, audio, telegram_client_id) -> Optional[Audio]:
        if audio is None or telegram_client_id is None:
            return None
        return Audio.find_by_key(
            self.doc_audios, Audio.get_key_from_audio(audio, telegram_client_id)
        )
